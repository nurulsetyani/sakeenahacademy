...
export async function createTahsinSchedule(formData: FormData) {
  ...
  if (!course) throw new Error("Kelas tidak ditemukan");
  ...
  revalidatePath("/admin/tahsin");
}

export async function saveTahsinAttendance(scheduleId: string, studentId: string, formData: FormData) {
  const supabase = await createClient();
  ...
  revalidatePath(`/admin/tahsin/${scheduleId}`);
}

export async function saveTahsinAssessment(scheduleId: string, studentId: string, formData: FormData) {
  const supabase = await createClient();
  const {
    data: { user },
  ...
  const { error } = await supabase.from("tahsin_assessments").upsert(
    {
